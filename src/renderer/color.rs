...
mod tests {
    ...
    fn test_rgb_roundtrip() {
        let original = (128, 64, 32);
        let packed = rgb_to_u32(original.0, original.1, original.2);
        let unpacked = u32_to_rgb(packed);
        assert_eq!(original, unpacked);
    }

    #[test]
    fn test_lighten_color() {
        let color = rgb_to_u32(100, 100, 100);
        let lighter = lighten_color(color);
        let (r, g, b) = u32_to_rgb(lighter);
        assert_eq!((r, g, b), (132, 132, 132));
    }

    #[test]
    fn test_lighten_color_saturates() {
        let color = rgb_to_u32(250, 250, 250);
        let lighter = lighten_color(color);
        let (r, g, b) = u32_to_rgb(lighter);
    ...
    fn test_color_to_u32_default() {
        let palette = create_palette();
        let result = color_to_u32(Color::Default, 0xAABBCC, &palette);
